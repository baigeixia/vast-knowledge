...
public class EsArticleSynChXxlJob {
    ...
    public void esArticleSync() {
        ...
        if (StringUtils.isEmpty(recordTime)) {
            log.info("执行定时任务esArticleSync: 全量同步未执行,此次不执行");
            XxlJobHelper.log("执行定时任务esArticleSync: 全量同步未执行,此次不执行");
            return;
        }
        if(SyncUtil.sync_status){
            log.info("执行定时任务esArticleSync: 全量同步已执行,但未完成，此次不执行");
            XxlJobHelper.log("执行定时任务esArticleSync: 全量同步已执行,但未完成，此次不执行");
            return;
        }
        log.info("执行定时任务esArticleSync:: 执行时间" + LocalDateTime.now());
        XxlJobHelper.log("执行定时任务esArticleSync:: 执行时间" + LocalDateTime.now());
        //格式化数据
        ...
        log.info("执行定时任务esArticleSync:: 需要同步的文章size={}, from={},to={}", apArticles==null?0:apArticles.size(),redisTime,now);
        XxlJobHelper.log("执行定时任务esArticleSync:: 需要同步的文章size={}, from={},to={}", apArticles==null?0:apArticles.size(),redisTime,now);
