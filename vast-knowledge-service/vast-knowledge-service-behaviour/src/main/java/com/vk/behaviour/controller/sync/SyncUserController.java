package com.vk.behaviour.controller.sync;
...
public class SyncUserController {
    ...
    public AjaxResult importAll() {
        ...
        Boolean flag = redisService.cacheObjectIfAbsent("recordUserReadTime", startTimeStr);
        log.info("全量同步start：{}, redisFlag={}",startTimeStr, flag);
        if (Boolean.TRUE.equals(flag)) {
            try {
                SyncUtil.sync_status = true;
                //1.查询统计所有数量
                Long total = apReadBehaviorService.selectCount(now);
                ...
                for (long page = 1L; page <= totalPages; page++) {
                    apReadBehaviorService.importAll(page, size, countDownLatch,now);
                }
