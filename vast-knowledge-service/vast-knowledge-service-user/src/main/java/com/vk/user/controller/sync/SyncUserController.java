package com.vk.user.controller.sync;


import com.vk.common.core.web.domain.AjaxResult;
import com.vk.common.redis.service.RedisService;
import com.vk.user.controller.ApUserController;
...
public class SyncUserController {
    ...
    public AjaxResult importAll() {
        ...
        Boolean flag = redisService.cacheObjectIfAbsent("recordUserTime", startTimeStr);
        log.info("全量同步start：{}, redisFlag={}",startTimeStr, flag);
        if (Boolean.TRUE.equals(flag)) {
            try {
                SyncUtil.sync_status = true;
                //1.查询统计所有数量
                Long total = apUserInfoService.selectCount(now);
                ...
                for (long page = 1L; page <= totalPages; page++) {
                    apUserInfoService.importAll(page, size, countDownLatch,now);
                }
